...
namespace erl::env {
    ...
    public:
        EnvironmentGridAnchor(
            ...
            std::shared_ptr<common::GridMapInfo<Dim>> grid_map_info)
        ...
        StateHashing(const std::shared_ptr<env::EnvironmentState> &env_state) const override {
            ERL_WARN_ONCE_COND(
                env_state->grid.size() > Dim,
                "only the first %d dimensions of grid state are used for hashing.",
                Dim);
        ...
        MetricToGrid(const Eigen::Ref<const Eigen::VectorXd> &metric_state) const override {
            ...
            for (int i = 0; i < Dim; ++i) {
                grid[i] = m_grid_map_info_->MeterToGridForValue(metric_state[i], i);
            }
            if (metric_state.size() > Dim) {
                ERL_WARN_ONCE(
                    "metric_state has more dimensions than grid map dimension, "
                    "the extra dimensions are casted to int directly, which may cause information "
                    "loss.");
        ...
        GridToMetric(const Eigen::Ref<const Eigen::VectorXi> &grid_state) const override {
            ...
            for (int i = 0; i < Dim; ++i) {
                metric[i] = m_grid_map_info_->GridToMeterForValue(grid_state[i], i);
            }
            if (grid_state.size() > Dim) {
                ERL_WARN_ONCE(
                    "grid_state has more dimensions than grid map dimension, the extra dimensions "
                    "are casted to double directly.");

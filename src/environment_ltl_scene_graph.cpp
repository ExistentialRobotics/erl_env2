...
namespace erl::env {
    ...
    EnvironmentLTLSceneGraph::ForwardAction(
        ...
        switch (level) {
            case scene_graph::Node::Type::kOcc: {  // atomic action
                ...
                if (std::size_t(atomic_action_id) <
                    m_atomic_actions_.size() - 2) {  // grid movement
    ...
    EnvironmentLTLSceneGraph::GetSuccessorsAtLevel(
        ...
        switch (level) {
            case scene_graph::Node::Type::kOcc: {
                ...
                if (floor_num_up < m_scene_graph_->num_floors &&
                    ...
                    if (!std::isinf(cost)) {
                        ...
                        if (dst_q.empty()) {
                            dst_q.resize(m_setting_->fsa->num_states, -1);
                        }  // initialize
                        nq = dst_q[cur_q];  // read from the cache
                        auto &path = m_up_stairs_path_maps_.at(
                ...
                if (floor_num_down >= 0 && floor->down_stairs_portal.has_value() &&
                    ...
                    nq = dst_q[cur_q];  // read from the cache
                    auto &path = m_down_stairs_path_maps_.at(
                        cur_z)(cur_x, cur_y);  // path to the downstairs portal
                    if (nq < 0) {              // not computed yet
                        nq = cur_q;
                        bool encounter_sink_state = false;
                        for (auto &point: path) {
            ...
            case scene_graph::Node::Type::kObject: {
                ...
                for (auto &itr: room->objects) {
                    ...
                    int &nq = next_env_state->grid[3];
                    auto &dst_q =
                        const_cast<std::vector<int> &>(m_object_path_q_maps_.at(object_id)(r, c));
                    if (dst_q.empty()) {
                        dst_q.resize(m_setting_->fsa->num_states, -1);
                    }  // initialize
                    nq = dst_q[cur_q];             // read from the cache
            ...
            case scene_graph::Node::Type::kFloor: {
                ...
                if (floor_num_up < m_scene_graph_->num_floors) {  // go upstairs
                    ...
                    if (!std::isinf(
                            cost)) {  // no path to go upstairs (should not happen, but just in case
                        auto next_env_state = std::make_shared<EnvironmentState>();
                        next_env_state->grid.resize(4);
                        int &nx = next_env_state->grid[0];
                        int &ny = next_env_state->grid[1];
    ...
    EnvironmentLTLSceneGraph::GenerateLabelMaps() {
        ...
        for (int i = 0; i < m_scene_graph_->num_floors; ++i) {
            label_maps[i].resize(m_grid_map_info_->Shape(0), m_grid_map_info_->Shape(1));
        }
...
#pragma omp parallel for collapse(2) default(none) \
    shared(rows, cols, num_propositions, label_map, i)
        ...
        ERL_INFO(
            "GenerateLabelMaps: {:f} ms",
            std::chrono::duration<double, std::milli>(t1 - t0).count());
    }

    bool
    EnvironmentLTLSceneGraph::EvaluateReachObject(
        int x,
        int y,
        int floor_num,
        int uuid,
    ...
    std::vector<std::shared_ptr<EnvironmentState>>
    EnvironmentLTLSceneGraph::ConvertPath(
        const std::vector<std::array<int, 2>> &path,
        int floor_num,
        int cur_q) const {
    ...
    std::vector<std::shared_ptr<EnvironmentState>>
    EnvironmentLTLSceneGraph::GetPathToFloor(
        int xg,
        int yg,
        int floor_num,
        int cur_q,
        int next_floor_num) const {
        ERL_DEBUG_ASSERT(

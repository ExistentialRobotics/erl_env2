...
TEST(ERL_ENV, SceneGraph) {
    ...
    EXPECT_EQ(floor->up_stairs_cost, 4.484692711177653);
    ...
    ASSERT_EIGEN_VECTOR_NEAR("grid_map_origin", floor->grid_map_origin, Eigen::Vector2d(-3.9349700000000003, -8.69993), 1e-15);
    ...
    cv::Mat room_map = building.LoadRoomMap(data_dir, 0);
    cv::Mat cat_map = building.LoadCatMap(data_dir, 0);
    double min, max;
    ...
    // cv::Mat mask(cat_map.rows, cat_map.cols, CV_32SC1, int(erl::env::scene_graph::Object::SOC::kGround));
    ...
    std::cout << "sub mask size: " << sub_mask.rows << " rows x " << sub_mask.cols << " cols" << std::endl;
    std::cout << "sub mask is subMatrix: " << sub_mask.isSubmatrix() << std::endl;  // "sub mask is subMatrix: 1
